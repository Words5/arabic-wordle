...
function checkGuess() {
  ...
  for (let ch of targetArr) {
    letterUsage[ch] = (letterUsage[ch] || 0) + 1;
  }
  ...
  // Step 2: Blue
  for (let i = 0; i < guessArr.length; i++) {
    if (colors[i] === "" && letterUsage[guessArr[i]] > 0 && guessArr.filter((g, idx) => g === guessArr[i] && colors[idx] === "correct").length === 0) {
      colors[i] = "repeat";
      letterUsage[guessArr[i]]--;
    }
  }
  ...
  // Step 3: Yellow
  for (let i = 0; i < guessArr.length; i++) {
    if (colors[i] === "" && targetWord.includes(guessArr[i]) && letterUsage[guessArr[i]] > 0) {
      colors[i] = "wrong-place";
      letterUsage[guessArr[i]]--;
    }
  }
